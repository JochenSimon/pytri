...
class Monipulator(QMainWindow):
  def __init__(self, app, host, port, console, bundle, info, open, quit, lightmode, zoom, config):
    ...
    self.setWindowTitle("Pytri Monipulator v" + __version__)
  ...
  def do_saveas(self):
    ...
    if not agentname.startswith("$"):
      ...
      filetype = ".pta"
      filetypehint = "Pytri Agent File (*.pta);;Pytri Fragment File (*.ptf)"
    else:
      agentname = agentname[1:]
      directory = "workbench/fragments" + ("/" if "." in agentname else "")
      filetype = ".ptf"
      filetypehint = "Pytri Fragment File (*.ptf);;Pytri Agent File (*.pta)"
